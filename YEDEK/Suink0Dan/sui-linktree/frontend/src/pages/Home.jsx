...
import { isZkLoginActive, getZkLoginSession } from '../utils/zklogin';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (isZkLoginActive()) {
      ...
      setZkLoginUser(session);
    }
  }, []);

  const loadProfiles = async () => {
    const address = currentAccount?.address || zkLoginUser?.address;
    if (!address) return;
    
    setLoading(true);
    try {
      const userProfiles = await getProfilesByOwner(address);
  ...
  useEffect(() => {
    if (currentAccount || zkLoginUser) {
      loadProfiles();
    }
  }, [currentAccount, zkLoginUser]);
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          {currentAccount || zkLoginUser ? (
            ...
            <div className="card p-8 max-w-md w-full">
              <p className="text-lg mb-4 text-center">Connect your wallet to get started</p>
